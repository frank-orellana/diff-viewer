...
type IFiles = Array<IFiles>;


export function diff(lines: string[]) {
	let line = lines.shift();
	let file: DiffFile = new DiffFile();
	const files: DiffFile[] = [];

	while (line != undefined) {
		let firstLine = line.trim();
		while (firstLine == '') firstLine = lines.shift();
		...
		if (firstLine.startsWith('diff')) {
			...
			file.A = new File(); file.B = new File();
